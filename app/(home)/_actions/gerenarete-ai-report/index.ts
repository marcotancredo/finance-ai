...
//TODO: marco.tancredo - ajustar para atribuir valores reais
const DUMMY_REPORT =
...
export const generateAiReport = async ({ month }: GenerateAiReportSchema) => {
  ...
  minha vida financeira. As transações estão dividas por ponto e vírgula. A estrutura de cada uma é {DATA}-{TIPO}-
  {VALOR}-{CATEGORIA}. São elas:
  ${transactions
    .map(
      (transaction) =>
        `${transaction.date.toLocaleDateString("pt-BR")}-R$${transaction.amount}-${transaction.type}-${
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um especialista em gestão e organização de finanças pessoais. Você ajuda as pessoas a organizaram melhor as suas finanças",
      },
      {
        role: "user",
        content,
